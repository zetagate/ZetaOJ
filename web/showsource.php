...
if (isset($_SESSION['user_id'])&& $row) {
	...
	$sql = "SELECT distinct `problem_id` FROM `solution` WHERE `user_id`=? AND `result`=4 and `problem_id`=?";
	$result=pdo_query($sql, $_SESSION['user_id'], $sproblem_id);
	if(count($result) == 1) {

		//is option of sharing true?
		$sql = "SELECT share FROM solution WHERE `solution_id`=? AND `result`=4";
		$result=pdo_query($sql, $id);
