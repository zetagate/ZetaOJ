...
for ($i=0;$i<$rows_cnt;$i++){
	...
	$view_status[$i][3]="";
        ...
        if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
                                $view_status[$i][3].= ($row['pass_rate']*100)."%</a>";
        ...
        if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
              ...
              if(!$lock||$lock_time>$row['in_date']||$row['user_id']==$_SESSION['user_id']){
        		...
        		if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
                                $view_status[$i][3].= ($row['pass_rate']*100)."%</span>";

                        if( isset($_SESSION['source_browser'])){

                                        $view_status[$i][3].= "<a href=comparesource.php?left=".$row['sim_s_id']."&right=".$row['solution_id']."  class='btn-info btn-xs'  target=original>".$row['sim_s_id']."(".$row['sim']."%)</a>";
                        }else{

                                        $view_status[$i][3].= "<span class='btn-info btn-xs'>".$row['sim_s_id']."</span>";
