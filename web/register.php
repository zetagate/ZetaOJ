...
$recap_res = $_POST["g-recaptcha-response"];
...
$recap_options = array(
	'http' => array (
		'method' => 'POST',
		'content' => http_build_query($recap_data)
	)
);


$recap_ctx=stream_context_create($recap_options);
$recap_verify=file_get_contents($recap_url, false, $recap_ctx);
$recap_success=json_decode($recap_verify);

if($recap_success->success==false) {
	$err_str=$err_str."Please check \'I am not a robot\' first!\\n";
	$err_cnt++;
}
