<?php
    ...
    require_once('./include/db_info.inc.php');
        ...
        if(isset( $_GET ['start'] ))
                ...
                $sql = "SELECT `user_id`,`nick`,`solved`,`submit` FROM `users` ORDER BY `solved` DESC,accesstime DESC  LIMIT  " . strval ( $rank ) . ",$page_size";
                ...
                if($scope){
                        ...
                        $sql="SELECT users.`user_id`,`nick`,s.`solved`,t.`submit` FROM `users`
                                ...
                                ORDER BY s.`solved` DESC, accesstime DESC LIMIT  0,50
                         ";
//                      echo $sql;
                }
		...
		$rank_sql="SELECT count(*) as `Rank` FROM `users` WHERE `solved`>?";	
		$rank_result=pdo_query($rank_sql, $row['solved']);
		$real_rank=intval($rank_result[0][0]);
			...
			$rank++;
                        $real_rank++; 
			if($i==0) {
				$view_rank[$i][0]= $real_rank;
				$prev_rank=$real_rank;
			}
			else if($row['solved']==$prev) {
				$view_rank[$i][0]= $prev_rank;
			}
			else {
				$real_rank = $rank;
				$view_rank[$i][0]= $real_rank;
			...
			$temp=0;
			$colored_id= colorId($row['user_id'], $view_rank[$i][0]);


			$view_rank[$i][1].=  "<div class=center><a href='userinfo.php?user=" .htmlentities ( $row['user_id'],ENT_QUOTES,"UTF-8") . "'>" . $colored_id . "</a></div>";




                        $view_rank[$i][2]=  "<div class=center>" . htmlentities ( $row['nick'] ,ENT_QUOTES,"UTF-8") ."</div>";
                        $view_rank[$i][2]=  "<div class=center>" .$row['nick'] ."</div>";
