...
if((~$OJ_LANGMASK)&(1<<$language)){
	...
	if (!isset($pid)){
	$sql="INSERT INTO solution(problem_id,user_id,in_date,language,ip,code_length,result,share)
		VALUES(?,?,NOW(),?,?,?,14,?)";
		$insert_id= pdo_query($sql,$id,$user_id,$language,$ip,$len,$share);
	}else{
	$sql="INSERT INTO solution(problem_id,user_id,in_date,language,ip,code_length,contest_id,num,result,share)
		VALUES(?,?,NOW(),?,?,?,?,?,14,?)";
		$insert_id= pdo_query($sql,$id,$user_id,$language,$ip,$len,$cid,$pid,$share);
	}
