...
void make_diff_out_simple(FILE *f1, FILE *f2, int c1, int c2, const char * path) {
	execute_cmd("echo '========[%s]========='>>diff.out",getFileNameFromPath(path));
	execute_cmd("echo '=======Answer, Your Answer====='>>diff.out");
	//execute_cmd("diff '%s' user.out|head -2>>diff.out",path);
	execute_cmd("diff -y '%s' user.out | grep '|' | sed 's/\\s*|//g' | head -1 >> diff.out",path);
	execute_cmd("echo '=============================='>>diff.out");
}
